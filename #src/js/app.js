@@include('files/_variables.js')
@@include('files/checkDevice.js')
@@include('files/menu.js')
/*@@include('files/tabs.js')*/
@@include('files/bodyLook.js')
@@include('files/popup.js')
@@include('files/headerFix.js')
@@include('files/calculator.js')
@@include('files/infrastructure-map.js')



//slider
@@include('files/sliders/portfolio.js')
@@include('files/sliders/confort.js')
@@include('files/sliders/about_us.js')
@@include('files/sliders/certificates.js')
@@include('files/sliders/warranty.js')
@@include('files/sliders/news.js')
@@include('files/sliders/img.js')
@@include('files/sliders/first.js')
@@include('files/sliders/map.js')
@@include('files/sliders/HC.js')
@@include('files/sliders/show-offs.js')
@@include('files/sliders/room.js')
@@include('files/sliders/doc.js')
@@include('files/sliders/house-sl.js')
@@include('files/sliders/infrastructure.js')
@@include('files/sliders/finishing.js')
//slider